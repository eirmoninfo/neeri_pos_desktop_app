...
const serviceTileRows = useMemo(() => {
  ...
  const renderMiscellaneousTile = (key = "miscellaneous-tile") => (
    <div
      key={key}
      className="flex min-h-[96px] w-full min-w-0 flex-col items-center justify-center gap-1.5 rounded-xl border-2 border-dashed border-violet-400 bg-violet-100 px-2 py-2 text-center"
      onClick={(event) => event.stopPropagation()}
    >
      <span className="text-[10px] font-extrabold uppercase leading-tight text-violet-900">Miscellaneous</span>
      <input
              ...
              {posBootLoading
                ...
                ? (
                  <>
                    ...
                    {renderMiscellaneousTile("miscellaneous-in-category")}
                  </>
                )
                : serviceTileRows.map((category) => {
                    const typedCategory = String(category);
                    if (typedCategory === "Miscellaneous") {
                      return renderMiscellaneousTile(typedCategory);
                    }
